...
export default function DentalClientPatients() {
  ...
  return <div className="min-h-[calc(100vh-4rem)] px-4 py-6 pb-24 sm:px-6 lg:px-8">
    ...
    {loading ? <div className="dental-stitch-card py-12 text-center text-sm text-[#727783]">Loading patients…</div> : <div className="grid gap-5 lg:grid-cols-2">{filtered.map((patient) => { const status = patientStatus(patient); const initials = `${patient.first_name[0] || ''}${patient.last_name[0] || ''}`; return <article key={patient.id} className="dental-stitch-card flex flex-col gap-4 p-5 transition hover:-translate-y-0.5"><div className="flex items-start justify-between gap-3"><div className="flex gap-4"><div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#d6e3ff] text-lg font-bold text-[#005db6]">{initials}</div><div><h2 className="text-xl font-semibold text-[#151c23]">{patient.first_name} {patient.last_name}</h2><p className="text-xs text-[#727783]">ID: #{patient.id.slice(0, 8)}</p></div></div><span className="rounded-full bg-[#5897f4]/20 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#00468b]">{status}</span></div><div className="flex items-center gap-2 border-y border-[#c2c6d4]/20 py-3 text-sm text-[#424752]"><Stethoscope size={18} className="text-[#005db6]" /> Last visit: {formatDate(patient.last_visit)}</div><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span className="flex items-center gap-2 text-xs font-bold text-[#151c23]"><Clock3 size={17} className="text-[#727783]" />Next: {formatDate(patient.next_appointment)}</span><div className="flex items-center gap-2"><button onClick={() => navigate(`/dental-client/patients/${encodeURIComponent(patient.id)}`)} className="rounded-xl bg-[#d6e3ff] px-3 py-2.5 text-xs font-bold text-[#005db6] hover:bg-[#c6d9ff]">View Profile</button><a href={`tel:${patient.phone || ''}`} aria-label={`Call ${patient.first_name}`} className="flex h-11 w-11 items-center justify-center rounded-full bg-[#e2e9f2] text-[#005db6]"><Phone size={18} /></a><button aria-label={`Message ${patient.first_name}`} className="flex h-11 w-11 items-center justify-center rounded-full bg-[#005db6] text-white shadow-md"><MessageCircle size={18} /></button></div></div></article> })}</div>}
