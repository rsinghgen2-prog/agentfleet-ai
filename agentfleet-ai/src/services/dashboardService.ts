import { ApiError } from '../utils/apiError'
import { API_BASE_URL, AUTH_API_URL } from '../config/api'

const DEMO_MODE = import.meta.env.VITE_DEMO_MODE === 'true'

export interface Patient { id: string; first_name: string; last_name: string; phone?: string; email?: string; gender?: string; date_of_birth?: string; notes?: string; medical_history?: Record<string, unknown>; allergies?: string[]; dental_history?: Record<string, unknown>; last_visit?: string | null; next_appointment?: string | null }
export interface Appointment { id: string; patient_id: string; appointment_date: string; appointment_time: string; duration: number; appointment_type: string; status: string; reason: string; notes: string; diagnosis?: string | null; treatment_plan?: Record<string, unknown> | null; first_name: string; last_name: string; phone: string; email?: string; gender: string }
export interface DentistNote { id: string; title: string; content: string; expires_at: string; is_active: boolean; created_by?: string | null; created_at: string; updated_at: string }
export interface DentistNoteInput { title: string; content: string; expiresAt?: string | null }
export interface InventoryItem { id: string; name: string; category: string; quantity: number; reorder_level: number; unit: string; is_active?: boolean; low_stock?: boolean }
export interface InventoryInput { name: string; category: string; quantity: number; reorderLevel: number; unit: string }
export type InventoryOrderStatus = 'draft' | 'placed' | 'confirmed' | 'partially_received' | 'received' | 'cancelled'
export type InventoryPaymentStatus = 'pending' | 'paid' | 'failed' | 'refunded'
export type InventoryPaymentMethod = 'cash' | 'cheque' | 'online' | 'bank_transfer'
export interface InventoryVendor { id: string; name: string; contact_name?: string | null; email?: string | null; phone?: string | null; address?: string; notes?: string; is_active?: boolean; created_at?: string; updated_at?: string }
export interface InventoryVendorInput { name: string; contactName?: string | null; email?: string | null; phone?: string | null; address?: string; notes?: string }
export interface InventoryOrderItem { id?: string; inventory_item_id?: string | null; item_name: string; quantity: number; unit: string; unit_price: number; total_price: number }
export interface InventoryOrder { id: string; vendor_id?: string | null; vendor_name?: string | null; vendor_email?: string | null; vendor_phone?: string | null; order_number: string; patient_id?: string | null; status: InventoryOrderStatus; payment_status: InventoryPaymentStatus; payment_method?: InventoryPaymentMethod | null; subtotal: number; tax_amount: number; total_amount: number; notes: string; expected_delivery_date?: string | null; placed_at?: string | null; created_by?: string | null; created_at: string; updated_at: string; items: InventoryOrderItem[]; events: InventoryOrderEvent[]; communications: InventoryCommunication[] }
export interface InventoryOrderInput { vendorId?: string | null; orderNumber?: string; patientId?: string | null; items: Array<{ inventoryItemId?: string | null; itemName: string; quantity: number; unit: string; unitPrice: number }>; taxAmount?: number; notes?: string; expectedDeliveryDate?: string | null; status?: InventoryOrderStatus; paymentStatus?: InventoryPaymentStatus; paymentMethod?: InventoryPaymentMethod | null }
export type InventoryOrderUpdateInput = Partial<InventoryOrderInput>
export interface InventoryOrderEvent { id: string; order_id: string; event_type: string; status?: InventoryOrderStatus | null; description: string; location: string; occurred_at: string; created_by?: string | null; created_at?: string }
export interface InventoryCommunication { id: string; order_id: string; channel: 'email' | 'sms'; recipient_type: 'vendor' | 'patient' | 'client'; recipient: string; subject: string; body: string; status: string; sent_at?: string | null; created_by?: string | null; created_at: string }
export interface InventoryCommunicationInput { channel: 'email' | 'sms'; recipientType?: 'vendor' | 'patient' | 'client'; recipient?: string; subject?: string; body: string; copyToPatient?: boolean; patientId?: string | null }
export interface SupportMessage { id: string; conversation_id: string; sender_id: string; sender_name?: string | null; sender_role: string; body: string; created_at: string }
export interface SupportConversation { id: string; subject: string; status: 'open' | 'closed'; created_by?: string | null; created_at: string; updated_at: string }
export interface SupportConversationSummary extends SupportConversation { requester_name?: string | null; last_message?: string | null }
export interface SupportChat { conversation: SupportConversation | null; messages: SupportMessage[] }
export interface HospitalDirectory { id: string; name: string; relationship: 'own' | 'partner'; specialty: string; address: string; city: string; contact_name?: string; contact_phone?: string; is_active?: boolean }
export interface NotificationAlert { id: string; kind: 'message' | 'email' | 'call'; title: string; body: string; customer_name?: string | null; is_read: boolean; created_at: string }
export interface ClinicSettings { clinic_name: string; clinic_email?: string; phone?: string; address: Record<string, unknown>; branding: Record<string, string>; working_hours: Record<string, string>; appointment_settings: Record<string, unknown>; notifications: Record<string, boolean>; timezone: string }
export interface DashboardData { todaysAppointments: Appointment[]; calendarData: Array<{ appointment_date: string; count: number }>; upcomingFollowUps?: Patient[]; stats: { todayVisits: number; newPatientsToday: number; totalAppointmentsToday: number; totalPatients: number }; currentDate: { month: number; year: number; today: string } }

const asCount = (...values: unknown[]) => {
  for (const value of values) {
    if (value === null || value === undefined || value === '') continue
    const parsed = Number(value)
    if (Number.isFinite(parsed)) return parsed
  }
  return 0
}

function normalizeDashboardData(raw: Record<string, unknown> | DashboardData | null | undefined): DashboardData {
  const source = (raw || {}) as Record<string, unknown>
  const stats = (source.stats || {}) as Record<string, unknown>
  const currentDate = (source.currentDate || source.current_date || {}) as Record<string, unknown>
  return {
    todaysAppointments: Array.isArray(source.todaysAppointments) ? source.todaysAppointments as Appointment[] : [],
    calendarData: Array.isArray(source.calendarData) ? source.calendarData as DashboardData['calendarData'] : Array.isArray(source.calendar_data) ? source.calendar_data as DashboardData['calendarData'] : [],
    upcomingFollowUps: Array.isArray(source.upcomingFollowUps) ? source.upcomingFollowUps as Patient[] : Array.isArray(source.upcoming_follow_ups) ? source.upcoming_follow_ups as Patient[] : [],
    stats: {
      todayVisits: asCount(stats.todayVisits, stats.today_visits),
      newPatientsToday: asCount(stats.newPatientsToday, stats.new_patients_today),
      totalAppointmentsToday: asCount(stats.totalAppointmentsToday, stats.total_appointments_today),
      totalPatients: asCount(stats.totalPatients, stats.total_patients),
    },
    currentDate: {
      month: asCount(currentDate.month, new Date().getMonth() + 1) || new Date().getMonth() + 1,
      year: asCount(currentDate.year, new Date().getFullYear()) || new Date().getFullYear(),
      today: typeof currentDate.today === 'string' && currentDate.today ? currentDate.today : `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}-${String(new Date().getDate()).padStart(2, '0')}`,
    },
  }
}
export interface PatientVisit { id: string; visit_date: string; appointment_time?: string; provider: string; visit_type: string; status: string; summary: string; procedures?: string[]; is_appointment?: boolean }
export interface PatientPrescription { id: string; patient_id: string; medication: string; dosage: string; frequency: string; duration: string; instructions: string; prescribed_at: string; prescribed_by: string }
export interface PatientMedicalReport { id: string; patient_id: string; file_name: string; mime_type: string; file_size: number; data_base64: string; description?: string; uploaded_at: string; uploaded_by: string }
export type LabDispatchStatus = 'queued' | 'provider-ready'
export interface PatientDispatchRecord { id: string; lab_order_id: string; channel: 'patient' | 'clinic' | 'lab'; recipient: string; status: LabDispatchStatus; created_at: string; note: string }
export interface PatientLabOrder { id: string; patient_id: string; order_number: string; ordered_at: string; tests: string; teeth_creation_service: string; lab_name: string; lab_email: string; lab_phone: string; instructions: string; copy_to_patient: boolean; copy_to_clinic: boolean; dispatch_status: LabDispatchStatus; dispatch_note: string; dispatch_history: PatientDispatchRecord[] }
export interface PatientProfile { patient: Patient; visits: PatientVisit[]; prescriptions: PatientPrescription[]; reports: PatientMedicalReport[]; lab_orders: PatientLabOrder[]; dispatch_history: PatientDispatchRecord[] }
export interface PrescriptionInput { patientId: string; medication: string; dosage: string; frequency: string; duration: string; instructions: string }
export interface PrescriptionUpdateInput extends PrescriptionInput { prescriptionId: string }
export interface PatientUpdateInput { firstName?: string; lastName?: string; email?: string | null; phone?: string | null; dateOfBirth?: string | null; gender?: string | null; notes?: string | null; medicalHistory?: Record<string, unknown>; allergies?: string[]; dentalHistory?: Record<string, unknown> }
export interface PatientCreateInput { firstName: string; lastName: string; email?: string | null; phone?: string | null; dateOfBirth?: string | null; gender?: string | null; notes?: string | null }
export interface MedicalReportInput { patientId: string; fileName: string; mimeType: string; fileSize: number; dataBase64: string; description?: string }
export interface LabOrderInput { patientId: string; orderNumber?: string; tests: string; teethCreationService: string; labName: string; labEmail: string; labPhone: string; instructions: string; copyToPatient: boolean; copyToClinic: boolean }
export type PaymentStatus = 'pending' | 'paid' | 'failed' | 'refunded'
export type PaymentMethod = 'cash' | 'cheque' | 'online' | 'bank_transfer' | 'card' | 'upi'
export interface Payment { id: string; customer_id: string; payment_number: string; amount: number; currency: string; status: PaymentStatus; method?: PaymentMethod | null; description: string; paid_at?: string | null; created_by?: string | null; created_at: string; updated_at: string; first_name: string; last_name: string; email?: string | null; phone?: string | null }
export interface PaymentInput { customerId: string; paymentNumber?: string; amount: number; currency?: string; status?: PaymentStatus; method?: PaymentMethod | null; description?: string }
export interface PaymentUpdateInput { amount?: number; currency?: string; status?: PaymentStatus; method?: PaymentMethod | null; description?: string }
export interface PaymentSummary { total_payments: number; total_amount: number; collected_amount: number; pending_amount: number; refunded_amount: number; paid_count: number; pending_count: number; customers_count: number }
export type InvoiceStatus = 'draft' | 'issued' | 'paid' | 'void' | 'overdue'
export interface InvoiceLineItem { description: string; quantity: number; unitPrice: number; amount: number }
export interface Invoice { id: string; customer_id: string; payment_id?: string | null; invoice_number: string; currency: string; status: InvoiceStatus; due_date?: string | null; notes: string; line_items: InvoiceLineItem[]; total_amount: number; created_at: string; updated_at: string; first_name: string; last_name: string; email?: string | null; phone?: string | null }
export interface InvoiceInput { customerId: string; invoiceNumber?: string; currency?: string; status?: InvoiceStatus; dueDate?: string | null; notes?: string; lineItems: Array<Omit<InvoiceLineItem, 'amount'> & { amount?: number }> }
export interface InvoiceUpdateInput { status?: InvoiceStatus; dueDate?: string | null; notes?: string; lineItems?: InvoiceInput['lineItems']; paymentId?: string | null }
export type MessageCampaignStatus = 'draft' | 'scheduled' | 'queued' | 'cancelled' | 'completed'
export interface MessageCampaignRecipient { name?: string; phone: string; email?: string | null }
export interface MessageCampaign { id: string; name: string; channel: 'whatsapp' | 'sms'; subject: string; message: string; recipients: MessageCampaignRecipient[]; recipient_count: number; status: MessageCampaignStatus; scheduled_at?: string | null; created_at: string; updated_at: string }
export interface MessageCampaignInput { name: string; channel: 'whatsapp' | 'sms'; subject?: string; message: string; recipients: MessageCampaignRecipient[]; scheduledAt?: string | null }
export type AppointmentStatus = 'scheduled' | 'confirmed' | 'in_progress' | 'completed' | 'cancelled' | 'no_show'
export interface AppointmentUpdateInput { patientId?: string; appointmentDate?: string; appointmentTime?: string; duration?: number; appointmentType?: string; status?: AppointmentStatus; reason?: string | null; notes?: string | null; diagnosis?: string | null; treatment_plan?: Record<string, unknown> | null; followUpRequired?: boolean; followUpDate?: string | null }
export type TreatmentPlanStatus = 'recommended' | 'accepted' | 'scheduled' | 'in_progress' | 'completed' | 'cancelled'
export type TreatmentPlanPriority = 'low' | 'medium' | 'high' | 'urgent'
export interface TreatmentPlan { id: string; patient_id: string; title: string; tooth: string; status: TreatmentPlanStatus; priority: TreatmentPlanPriority; estimated_cost?: number | null; due_date?: string | null; notes: string; created_at: string; updated_at: string }
export interface TreatmentPlanInput { title: string; tooth?: string; status?: TreatmentPlanStatus; priority?: TreatmentPlanPriority; estimatedCost?: number | null; dueDate?: string | null; notes?: string }
export type ToothRecordStatus = 'active' | 'resolved' | 'monitoring'
export interface ToothRecord { id: string; patient_id: string; tooth_number: number; conditions: string[]; pain_level: number; status: ToothRecordStatus; notes: string; created_at: string; updated_at: string }
export interface ToothRecordInput { conditions?: string[]; painLevel?: number; status?: ToothRecordStatus; notes?: string }

function normalizePatientProfile(raw: { patient: Patient; appointments?: Array<Record<string, unknown>>; prescriptions?: Array<Record<string, unknown>>; reports?: Array<Record<string, unknown>>; labOrders?: Array<Record<string, unknown>>; dispatchHistory?: Array<Record<string, unknown>> }): PatientProfile {
  const appointments = raw.appointments || []
  const visits: PatientVisit[] = appointments.map((visit) => ({ id: String(visit.id), visit_date: String(visit.appointment_date || ''), appointment_time: visit.appointment_time ? String(visit.appointment_time) : undefined, provider: String(visit.provider || 'Clinic team'), visit_type: String(visit.appointment_type || 'Dental visit'), status: String(visit.status || 'scheduled'), summary: String(visit.reason || visit.notes || 'Visit recorded.'), procedures: Array.isArray(visit.procedures) ? visit.procedures as string[] : [], is_appointment: true }))
  const patient = { ...raw.patient, last_visit: raw.patient.last_visit || visits.filter((visit) => visit.status === 'completed').map((visit) => visit.visit_date).sort().at(-1) || null, next_appointment: raw.patient.next_appointment || visits.filter((visit) => !['cancelled', 'completed'].includes(visit.status) && visit.visit_date >= today).map((visit) => visit.visit_date).sort()[0] || null }
  const prescriptions: PatientPrescription[] = (raw.prescriptions || []).map((item) => ({ id: String(item.id), patient_id: String(item.patient_id || patient.id), medication: String(item.medication || item.medication_name || ''), dosage: String(item.dosage || ''), frequency: String(item.frequency || ''), duration: String(item.duration || ''), instructions: String(item.instructions || 'Use as directed.'), prescribed_at: String(item.prescribed_at || item.created_at || ''), prescribed_by: String(item.prescribed_by || 'Clinic team') }))
  const reports: PatientMedicalReport[] = (raw.reports || []).map((item) => ({ id: String(item.id), patient_id: String(item.patient_id || patient.id), file_name: String(item.file_name || item.attachment_name || 'Clinical report'), mime_type: String(item.mime_type || item.attachment_mime_type || 'application/octet-stream'), file_size: Number(item.file_size || item.attachment_size || 0), data_base64: String(item.data_base64 || ''), description: item.description ? String(item.description) : undefined, uploaded_at: String(item.uploaded_at || item.created_at || ''), uploaded_by: String(item.uploaded_by || 'Clinic team') }))
  const normalizeDispatch = (item: Record<string, unknown>): PatientDispatchRecord => ({ id: String(item.id), lab_order_id: String(item.lab_order_id || ''), channel: item.recipient_type === 'patient' ? 'patient' : item.recipient_type === 'clinic' ? 'clinic' : 'lab', recipient: String(item.recipient || ''), status: ['sent', 'processing'].includes(String(item.status)) ? 'provider-ready' : 'queued', created_at: String(item.created_at || ''), note: item.note ? String(item.note) : `Queued via ${String(item.channel || 'provider')} for provider processing.` })
  const allDispatches = (raw.dispatchHistory || []).map(normalizeDispatch)
  const lab_orders: PatientLabOrder[] = (raw.labOrders || []).map((item) => { const dispatch_history = Array.isArray(item.dispatch_history) ? (item.dispatch_history as Array<Record<string, unknown>>).map(normalizeDispatch) : allDispatches.filter((dispatch) => dispatch.lab_order_id === String(item.id)); return { id: String(item.id), patient_id: String(item.patient_id || patient.id), order_number: String(item.order_number || `LAB-${String(item.id).slice(0, 8)}`), ordered_at: String(item.ordered_at || item.created_at || ''), tests: String(item.tests || item.test_name || ''), teeth_creation_service: String(item.teeth_creation_service || ''), lab_name: String(item.lab_name || ''), lab_email: String(item.lab_email || ''), lab_phone: String(item.lab_phone || ''), instructions: String(item.instructions || ''), copy_to_patient: Boolean(item.copy_to_patient ?? true), copy_to_clinic: Boolean(item.copy_to_clinic ?? false), dispatch_status: item.dispatch_status === 'provider-ready' || item.status === 'processing' ? 'provider-ready' : 'queued', dispatch_note: String(item.dispatch_note || 'Queued for provider processing; delivery status is traceable in Dispatch history.'), dispatch_history } })
  return { patient, visits, prescriptions, reports, lab_orders, dispatch_history: allDispatches }
}

function makeLabOrderPdf(input: { orderNumber: string; patientId: string; tests: string; teethService: string; labName: string; instructions: string }) {
  const safe = (value: string) => value.replace(/[^\x20-\x7E]/g, '?').replace(/[()\\]/g, (character) => `\\${character}`)
  const lines = [`Dental laboratory order`, `Order: ${input.orderNumber}`, `Patient ID: ${input.patientId}`, `Lab: ${input.labName}`, `Tests: ${input.tests}`, `Teeth service: ${input.teethService || 'Not specified'}`, `Instructions: ${input.instructions || 'None provided'}`]
  const stream = [`BT`, `/F1 16 Tf`, `50 760 Td`, `(${safe(lines[0])}) Tj`, `/F1 10 Tf`, ...lines.slice(1).map((line) => `0 -28 Td (${safe(line)}) Tj`), `ET`, ''].join('\n')
  const objects = [`<< /Type /Catalog /Pages 2 0 R >>`, `<< /Type /Pages /Kids [3 0 R] /Count 1 >>`, `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 5 0 R >> >> /Contents 4 0 R >>`, `<< /Length ${stream.length} >>\nstream\n${stream}endstream`, `<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>`]
  let pdf = '%PDF-1.4\n'; const offsets = [0]
  objects.forEach((object, index) => { offsets.push(pdf.length); pdf += `${index + 1} 0 obj\n${object}\nendobj\n` })
  const xref = pdf.length; pdf += `xref\n0 ${objects.length + 1}\n0000000000 65535 f \n${offsets.slice(1).map((offset) => `${String(offset).padStart(10, '0')} 00000 n \n`).join('')}trailer\n<< /Size ${objects.length + 1} /Root 1 0 R >>\nstartxref\n${xref}\n%%EOF\n`
  return { base64: btoa(pdf), size: new TextEncoder().encode(pdf).length }
}

function makePaymentReportPdf(input: { clinicName: string; generatedAt: string; summary: PaymentSummary; payments: Payment[] }) {
  const safe = (value: string) => value.replace(/[^\x20-\x7E]/g, '?').replace(/[()\\]/g, (character) => `\\${character}`)
  const currency = input.payments[0]?.currency || 'INR'
  const amount = (value: number) => `${currency} ${Number(value || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  const lines = [
    `Payment report - ${input.clinicName}`,
    `Generated: ${input.generatedAt}`,
    `Total payments: ${input.summary.total_payments}   Customers: ${input.summary.customers_count}`,
    `Collected: ${amount(input.summary.collected_amount)}   Pending: ${amount(input.summary.pending_amount)}   Refunded: ${amount(input.summary.refunded_amount)}`,
    ``,
    `Payment ID              Customer ID           Customer            Status     Amount`,
    ...input.payments.slice(0, 30).map((payment) => `${payment.payment_number.padEnd(22).slice(0, 22)}  ${payment.customer_id.slice(0, 18).padEnd(18)}  ${`${payment.first_name} ${payment.last_name}`.slice(0, 18).padEnd(18)}  ${payment.status.padEnd(9)}  ${amount(payment.amount)}`),
  ]
  const stream = [`BT`, `/F1 14 Tf`, `40 770 Td`, `(${safe(lines[0])}) Tj`, `/F1 8 Tf`, ...lines.slice(1).map((line) => `0 -18 Td (${safe(line)}) Tj`), `ET`, ''].join('\n')
  const objects = [`<< /Type /Catalog /Pages 2 0 R >>`, `<< /Type /Pages /Kids [3 0 R] /Count 1 >>`, `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 5 0 R >> >> /Contents 4 0 R >>`, `<< /Length ${stream.length} >>\nstream\n${stream}endstream`, `<< /Type /Font /Subtype /Type1 /BaseFont /Courier >>`]
  let pdf = '%PDF-1.4\n'; const offsets = [0]
  objects.forEach((object, index) => { offsets.push(pdf.length); pdf += `${index + 1} 0 obj\n${object}\nendobj\n` })
  const xref = pdf.length; pdf += `xref\n0 ${objects.length + 1}\n0000000000 65535 f \n${offsets.slice(1).map((offset) => `${String(offset).padStart(10, '0')} 00000 n \n`).join('')}trailer\n<< /Size ${objects.length + 1} /Root 1 0 R >>\nstartxref\n${xref}\n%%EOF\n`
  return { base64: btoa(pdf), size: new TextEncoder().encode(pdf).length }
}

const localDateKey = (date: Date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`
const today = localDateKey(new Date())
const dateIn = (days: number) => { const date = new Date(); date.setDate(date.getDate() + days); return localDateKey(date) }
const DEMO_PATIENTS_KEY = 'agentfleet.demo.patients'
const DEMO_APPOINTMENTS_KEY = 'agentfleet.demo.appointments'
const DEMO_DENTIST_NOTES_KEY = 'agentfleet.demo.dentistNotes'
const DEMO_INVENTORY_KEY = 'agentfleet.demo.inventory'
const DEMO_INVENTORY_VENDORS_KEY = 'agentfleet.demo.inventoryVendors'
const DEMO_INVENTORY_ORDERS_KEY = 'agentfleet.demo.inventoryOrders'
const DEMO_SUPPORT_CHAT_KEY = 'agentfleet.demo.supportChat'
const DEMO_CLIENT_ALERTS_KEY = 'agentfleet.demo.clientAlerts'
const DEMO_PATIENT_PROFILES_KEY = 'agentfleet.demo.patientProfiles'
const DEMO_PAYMENTS_KEY = 'agentfleet.demo.payments'
const mockPatients: Patient[] = [
  ['Aarav', 'Sharma', 'Active orthodontic treatment'], ['Meera', 'Cooper', 'Upcoming follow-up'], ['Kabir', 'Alexander', 'Needs filling review'], ['Ananya', 'Wilson', 'New patient'], ['Rohan', 'Fox', 'Requires treatment plan'],
  ['Isha', 'Howard', 'Preventive care'], ['Vihaan', 'Williamson', 'Implant review'], ['Tara', 'Simmons', 'Follow-up needed'], ['Dev', 'Patel', 'Overdue cleaning'], ['Zoya', 'Khan', 'Minor patient; guardian consent required'],
].map(([first_name, last_name, notes], index) => ({ id: `demo-patient-${index + 1}`, first_name, last_name, notes, phone: `+91-900000000${index + 1}`, email: `${first_name.toLowerCase()}@example.test`, gender: index % 2 ? 'Female' : 'Male', last_visit: index === 3 ? null : dateIn(-(index + 2)), next_appointment: [1, 4, 6, 7, 9].includes(index) ? dateIn(index + 3) : null }))
const mockAppointments: Appointment[] = [
  ['08:00:00', 0, 'Checkup', 'Regular checkup'], ['10:00:00', 1, 'Cleaning', 'Preventive cleaning'], ['14:00:00', 2, 'Cavity Filling', 'Filling review'], ['16:00:00', 3, 'Consultation', 'New patient assessment'],
].map(([appointment_time, patientIndex, appointment_type, reason], index) => { const patient = mockPatients[Number(patientIndex)]; return { id: `demo-appointment-${index + 1}`, patient_id: patient.id, appointment_date: today, appointment_time: String(appointment_time), duration: 30, appointment_type: String(appointment_type), status: index === 2 ? 'in_progress' : 'scheduled', reason: String(reason), notes: '', first_name: patient.first_name, last_name: patient.last_name, phone: patient.phone || '', gender: patient.gender || '' } })
const mockSettings: ClinicSettings = { clinic_name: 'V.P.S. Dental & Oral Care', clinic_email: 'info@vpsdental.com', phone: '+91-XXXXXXXXXX', address: { city: 'Kanpur', state: 'Uttar Pradesh', country: 'India' }, branding: { primaryColor: '#005db6', accentColor: '#a23858', logo: '🦷' }, working_hours: { monday: '09:00-18:00', saturday: '10:00-14:00' }, appointment_settings: { duration: 45, bufferMinutes: 10, emergencySlots: 2 }, notifications: { emailAlerts: true, smsReminders: true }, timezone: 'Asia/Kolkata' }
const mockInventory: InventoryItem[] = [
  { id: 'demo-1', name: 'Dental Examination Kit', category: 'Diagnostic', quantity: 24, reorder_level: 10, unit: 'kits', is_active: true, low_stock: false },
  { id: 'demo-2', name: 'Composite Resin A2', category: 'Restorative', quantity: 8, reorder_level: 10, unit: 'syringes', is_active: true, low_stock: true },
  { id: 'demo-3', name: 'Nitrile Examination Gloves', category: 'Consumables', quantity: 12, reorder_level: 15, unit: 'boxes', is_active: true, low_stock: true },
]
const mockInventoryVendors: InventoryVendor[] = [
  { id: 'demo-vendor-1', name: 'DentalCare Supplies', contact_name: 'Priya Mehta', email: 'orders@dentalcare.example', phone: '+91-9000001001', address: 'Kanpur Industrial Estate', notes: 'Primary consumables supplier', is_active: true },
  { id: 'demo-vendor-2', name: 'OrthoPro Distributors', contact_name: 'Arjun Kapoor', email: 'sales@orthopro.example', phone: '+91-9000001002', address: 'Lucknow Medical Market', notes: 'Orthodontic equipment', is_active: true },
]
const mockHospitals: HospitalDirectory[] = [
  { id: 'hospital-own-1', name: 'V.P.S. Dental & Oral Care', relationship: 'own', specialty: 'Dental & Oral Care', address: '12 Civil Lines', city: 'Kanpur', contact_name: 'Dr. Rajeev Pratap Singh', contact_phone: '+91 90000 00001', is_active: true },
  { id: 'hospital-partner-1', name: 'Regency Superspeciality Hospital', relationship: 'partner', specialty: 'Oral Surgery & Maxillofacial Care', address: 'A-2 Sarvodaya Nagar', city: 'Kanpur', contact_name: 'Referral Desk', contact_phone: '+91 90000 00002', is_active: true },
  { id: 'hospital-partner-2', name: 'Kalyanpur Diagnostic Centre', relationship: 'partner', specialty: 'Imaging & Diagnostics', address: 'Main GT Road, Kalyanpur', city: 'Kanpur', contact_name: 'Coordination Desk', contact_phone: '+91 90000 00003', is_active: true },
]
const mockClientAlerts: NotificationAlert[] = [
  { id: 'demo-alert-1', kind: 'message', title: 'New patient message', body: 'Meera Cooper asked whether her follow-up can be moved to Friday.', customer_name: 'Meera Cooper', is_read: false, created_at: new Date(Date.now() - 18 * 60 * 1000).toISOString() },
  { id: 'demo-alert-2', kind: 'email', title: 'Email from Aarav Sharma', body: 'Aarav shared an updated insurance document for his appointment.', customer_name: 'Aarav Sharma', is_read: false, created_at: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString() },
  { id: 'demo-alert-3', kind: 'call', title: 'Missed call from Kabir Alexander', body: 'A customer called the clinic and requested a callback.', customer_name: 'Kabir Alexander', is_read: true, created_at: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString() },
]

const demoProfileFor = (patient: Patient, index: number): PatientProfile => {
  const createdAt = new Date(Date.now() - (index + 1) * 86400000).toISOString()
  const visit: PatientVisit = { id: `demo-visit-${index + 1}`, visit_date: patient.last_visit || dateIn(-(index + 2)), provider: 'Dr. Rajeev Pratap Singh', visit_type: index % 2 ? 'Preventive check-up' : 'Treatment review', status: 'completed', summary: index % 2 ? 'Routine oral examination and cleaning completed.' : 'Reviewed treatment progress and updated care plan.', procedures: index % 2 ? ['Examination', 'Prophylaxis'] : ['Clinical assessment'] }
  const prescription: PatientPrescription = { id: `demo-prescription-${index + 1}`, patient_id: patient.id, medication: index % 2 ? 'Chlorhexidine mouthwash' : 'Fluoride toothpaste', dosage: index % 2 ? '10 ml' : 'Pea-sized amount', frequency: index % 2 ? 'Twice daily' : 'At bedtime', duration: '14 days', instructions: 'Use as directed and report any sensitivity or adverse reaction.', prescribed_at: createdAt, prescribed_by: 'Dr. Rajeev Pratap Singh' }
  const report: PatientMedicalReport = { id: `demo-report-${index + 1}`, patient_id: patient.id, file_name: index % 2 ? 'bitewing-review.txt' : 'oral-examination-notes.txt', mime_type: 'text/plain', file_size: 48, data_base64: 'U2FtcGxlIGNsaW5pY2FsIHJlY29yZCBmb3IgZGVtbyBtb2RlLg==', description: 'Sample clinical record for demo mode.', uploaded_at: createdAt, uploaded_by: 'V.P.S. Dental & Oral Care' }
  const sampleDispatch: PatientDispatchRecord = { id: `demo-dispatch-${index + 1}`, lab_order_id: `demo-lab-order-${index + 1}`, channel: 'lab', recipient: 'BrightSmile Dental Lab', status: 'provider-ready', created_at: createdAt, note: 'Prepared for provider handoff; no external delivery was attempted.' }
  const sampleLab: PatientLabOrder = { id: sampleDispatch.lab_order_id, patient_id: patient.id, order_number: `LAB-DEMO-${index + 1}`, ordered_at: createdAt, tests: 'Bitewing radiographs', teeth_creation_service: 'Crown shade matching', lab_name: 'BrightSmile Dental Lab', lab_email: 'orders@brightsmile.example', lab_phone: '+91 90000 01001', instructions: 'Confirm shade with clinic before fabrication.', copy_to_patient: true, copy_to_clinic: true, dispatch_status: 'provider-ready', dispatch_note: 'Provider-ready for manual handoff. This demo does not claim real provider delivery.', dispatch_history: [sampleDispatch] }
  return { patient, visits: [visit], prescriptions: [prescription], reports: [report], lab_orders: index === 0 ? [sampleLab] : [], dispatch_history: index === 0 ? [sampleDispatch] : [] }
}

function readDemoCollection<T>(key: string, fallback: T[]): T[] {
  try {
    const stored = JSON.parse(localStorage.getItem(key) || 'null')
    return Array.isArray(stored) ? stored as T[] : fallback
  } catch {
    return fallback
  }
}

function writeDemoCollection<T>(key: string, items: T[]) {
  try { localStorage.setItem(key, JSON.stringify(items)) } catch { /* Demo mode remains usable when storage is unavailable or full. */ }
}

const getDemoPatients = () => readDemoCollection<Patient>(DEMO_PATIENTS_KEY, mockPatients)
const getDemoPayments = (): Payment[] => readDemoCollection<Payment>(DEMO_PAYMENTS_KEY, [])
const demoPaymentWithCustomer = (payment: Payment): Payment => { const customer = getDemoPatients().find((patient) => patient.id === payment.customer_id); return { ...payment, first_name: customer?.first_name || payment.first_name || 'Unknown', last_name: customer?.last_name || payment.last_name || 'Customer', email: customer?.email ?? payment.email ?? null, phone: customer?.phone ?? payment.phone ?? null } }
const getDemoAppointments = () => readDemoCollection<Appointment>(DEMO_APPOINTMENTS_KEY, mockAppointments)
const getDemoAlerts = () => readDemoCollection<NotificationAlert>(DEMO_CLIENT_ALERTS_KEY, mockClientAlerts)
const getDemoPatientProfiles = () => {
  const patients = getDemoPatients()
  const stored = readDemoCollection<PatientProfile>(DEMO_PATIENT_PROFILES_KEY, [])
  const profiles = patients.map((patient, index) => stored.find((profile) => profile.patient.id === patient.id) || demoProfileFor(patient, index))
  if (stored.length !== profiles.length) writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles)
  return profiles
}
const demoAppointmentVisit = (appointment: Appointment): PatientVisit => ({ id: appointment.id, visit_date: appointment.appointment_date, appointment_time: appointment.appointment_time, provider: 'Clinic team', visit_type: appointment.appointment_type, status: appointment.status, summary: appointment.reason || appointment.notes || 'Appointment recorded.', procedures: [], is_appointment: true })
const getDemoPatientProfile = (id: string) => {
  const profile = getDemoPatientProfiles().find((item) => item.patient.id === id)
  if (!profile) throw new Error('Patient profile not found')
  const appointments = getDemoAppointments().filter((item) => item.patient_id === id)
  const appointmentIds = new Set(appointments.map((item) => item.id))
  const visits = [...appointments.map(demoAppointmentVisit), ...profile.visits.filter((visit) => !appointmentIds.has(visit.id))]
  const nextAppointment = appointments.filter((item) => !['cancelled', 'completed'].includes(item.status) && item.appointment_date >= today).sort((a, b) => `${a.appointment_date} ${a.appointment_time}`.localeCompare(`${b.appointment_date} ${b.appointment_time}`))[0]
  return { ...profile, patient: { ...profile.patient, next_appointment: nextAppointment?.appointment_date || null }, visits }
}
const inventoryWithStock = (item: InventoryItem): InventoryItem => ({ ...item, is_active: item.is_active !== false, low_stock: item.quantity <= item.reorder_level })
const getDemoInventory = (search = '', includeInactive = false) => {
  const stored = readDemoCollection<InventoryItem>(DEMO_INVENTORY_KEY, mockInventory).map(inventoryWithStock)
  const needle = search.trim().toLowerCase()
  return stored.filter((item) => (includeInactive || item.is_active !== false) && `${item.name} ${item.category}`.toLowerCase().includes(needle))
}
const getDemoInventoryVendors = (search = '') => { const needle = search.trim().toLowerCase(); return readDemoCollection<InventoryVendor>(DEMO_INVENTORY_VENDORS_KEY, mockInventoryVendors).filter((vendor) => vendor.is_active !== false && `${vendor.name} ${vendor.contact_name || ''} ${vendor.email || ''} ${vendor.phone || ''}`.toLowerCase().includes(needle)) }
const demoOrderItem = (item: InventoryOrderInput['items'][number], index: number): InventoryOrderItem => ({ id: `demo-order-item-${Date.now()}-${index}`, inventory_item_id: item.inventoryItemId || null, item_name: item.itemName, quantity: item.quantity, unit: item.unit, unit_price: item.unitPrice, total_price: item.quantity * item.unitPrice })
const getDemoInventoryOrders = (options: { search?: string; status?: InventoryOrderStatus; history?: boolean } = {}) => { const needle = (options.search || '').trim().toLowerCase(); return readDemoCollection<InventoryOrder>(DEMO_INVENTORY_ORDERS_KEY, []).map((order) => ({ ...order, items: Array.isArray(order.items) ? order.items : [], events: Array.isArray(order.events) ? order.events : [], communications: Array.isArray(order.communications) ? order.communications : [] })).filter((order) => (!options.history || order.status !== 'draft') && (!options.status || order.status === options.status) && (!needle || `${order.order_number} ${order.vendor_name || ''}`.toLowerCase().includes(needle))) }
const oneMonthFrom = (base = new Date()) => { const expiration = new Date(base); expiration.setMonth(expiration.getMonth() + 1); return expiration.toISOString() }
const getDemoDentistNotes = () => {
  const stored = readDemoCollection<DentistNote>(DEMO_DENTIST_NOTES_KEY, [])
  const now = Date.now()
  const normalized = stored.map((note) => ({ ...note, expires_at: note.expires_at || oneMonthFrom(note.created_at ? new Date(note.created_at) : new Date()), is_active: note.is_active !== false }))
  const updated = normalized.map((note) => note.is_active && Date.parse(note.expires_at) <= now ? { ...note, is_active: false } : note)
  if (JSON.stringify(stored) !== JSON.stringify(updated)) writeDemoCollection(DEMO_DENTIST_NOTES_KEY, updated)
  return updated.filter((note) => note.is_active)
}

function getDemoDashboardData(): DashboardData {
  const patients = getDemoPatients()
  const appointments = getDemoAppointments()
  const todaysAppointments = appointments.filter((appointment) => appointment.appointment_date === today)
  const counts = new Map<string, number>()
  appointments.forEach((appointment) => counts.set(appointment.appointment_date, (counts.get(appointment.appointment_date) || 0) + 1))
  return {
    todaysAppointments,
    calendarData: [...counts.entries()].map(([appointment_date, count]) => ({ appointment_date, count })),
    upcomingFollowUps: patients.filter((patient) => patient.next_appointment).slice(0, 10),
    stats: {
      todayVisits: new Set(todaysAppointments.map((appointment) => appointment.patient_id)).size,
      newPatientsToday: patients.filter((patient) => todaysAppointments.some((appointment) => appointment.patient_id === patient.id && !patient.last_visit)).length,
      totalAppointmentsToday: todaysAppointments.length,
      totalPatients: patients.length,
    },
    currentDate: { month: new Date().getMonth() + 1, year: new Date().getFullYear(), today },
  }
}

export class DashboardService {
  private static refreshInFlight: Promise<boolean> | null = null

  private static async refreshAccessToken() {
    if (this.refreshInFlight) return this.refreshInFlight
    this.refreshInFlight = (async () => {
      const refreshToken = localStorage.getItem('refreshToken')
      if (!refreshToken) return false
      const response = await fetch(`${AUTH_API_URL}/api/v1/auth/refresh`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ refreshToken }) })
      const body = await response.json().catch(() => ({}))
      if (!response.ok || !body.data?.accessToken) {
        localStorage.removeItem('accessToken')
        localStorage.removeItem('refreshToken')
        return false
      }
      localStorage.setItem('accessToken', body.data.accessToken)
      return true
    })()
    try { return await this.refreshInFlight } finally { this.refreshInFlight = null }
  }

  private static async fetchJson(path: string, options: RequestInit = {}, retried = false): Promise<{ ok: boolean; status: number; body: Record<string, unknown> }> {
    let response: Response
    try {
      const token = localStorage.getItem('accessToken')
      response = await fetch(`${API_BASE_URL}${path}`, { ...options, headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}), ...options.headers } })
    } catch (error) {
      throw new ApiError('The clinic API is unreachable. Confirm the patient service is running and VITE_API_URL is set.', 0)
    }
    const body = await response.json().catch(() => ({})) as Record<string, unknown>
    if (response.status === 401 && !retried && await this.refreshAccessToken()) return this.fetchJson(path, options, true)
    return { ok: response.ok, status: response.status, body }
  }

  private static async request<T>(path: string, options: RequestInit = {}): Promise<T> {
    const { ok, status, body } = await this.fetchJson(path, options)
    if (!ok) throw new ApiError(typeof body.message === 'string' ? body.message : `Request failed (${status || 'network'})`, status)
    if (!Object.prototype.hasOwnProperty.call(body, 'data')) throw new ApiError('API returned an invalid response', status)
    return body.data as T
  }

  static async getDashboardData(): Promise<DashboardData> { try { return normalizeDashboardData(await this.request<Record<string, unknown>>('/api/v1/patients/dashboard')) } catch (error) { if (!DEMO_MODE) throw error; console.warn('Using explicit demo dashboard data', error); return getDemoDashboardData() } }
  static async getPatients(search = '', limit = 25, offset = 0): Promise<{ data: Patient[]; total: number }> { try { const query = new URLSearchParams({ limit: String(limit), offset: String(offset), ...(search ? { search } : {}) }); const response = await this.requestBody<{ data: Patient[]; meta?: { total?: number } }>(`/api/v1/patients/patients?${query}`); return { data: response.data, total: response.meta?.total ?? response.data.length } } catch (error) { if (!DEMO_MODE) throw error; const needle = search.toLowerCase(); const filtered = getDemoPatients().filter((patient) => `${patient.first_name} ${patient.last_name} ${patient.phone} ${patient.email}`.toLowerCase().includes(needle)); return { data: filtered.slice(offset, offset + limit), total: filtered.length } } }
  static async createPatient(input: PatientCreateInput): Promise<Patient> {
    try {
      return await this.request<Patient>('/api/v1/patients/patients', { method: 'POST', body: JSON.stringify(input) })
    } catch (error) {
      if (!DEMO_MODE) throw error
      const patient: Patient = { id: `demo-patient-${Date.now()}`, first_name: input.firstName, last_name: input.lastName, phone: input.phone || undefined, email: input.email || undefined, gender: input.gender || undefined, date_of_birth: input.dateOfBirth || undefined, notes: input.notes || undefined, last_visit: null, next_appointment: null }
      writeDemoCollection(DEMO_PATIENTS_KEY, [patient, ...getDemoPatients()])
      return patient
    }
  }
  static async updatePatient(id: string, changes: PatientUpdateInput): Promise<Patient> { try { return await this.request<Patient>(`/api/v1/patients/patients/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(changes) }) } catch (error) { if (!DEMO_MODE) throw error; const current = getDemoPatients(); const existing = current.find((item) => item.id === id); if (!existing) throw new Error('Patient not found'); const updated = { ...existing, first_name: changes.firstName ?? existing.first_name, last_name: changes.lastName ?? existing.last_name, email: changes.email === undefined ? existing.email : changes.email || undefined, phone: changes.phone === undefined ? existing.phone : changes.phone || undefined, date_of_birth: changes.dateOfBirth === undefined ? existing.date_of_birth : changes.dateOfBirth || undefined, notes: changes.notes === undefined ? existing.notes : changes.notes || undefined }; writeDemoCollection(DEMO_PATIENTS_KEY, current.map((item) => item.id === id ? updated : item)); const profiles = getDemoPatientProfiles().map((item) => item.patient.id === id ? { ...item, patient: { ...item.patient, ...updated } } : item); writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles); return updated } }
  static async getPatientProfile(id: string): Promise<PatientProfile> { try { const raw = await this.request<{ patient: Patient; appointments?: Array<Record<string, unknown>>; prescriptions?: Array<Record<string, unknown>>; reports?: Array<Record<string, unknown>>; labOrders?: Array<Record<string, unknown>>; dispatchHistory?: Array<Record<string, unknown>> }>(`/api/v1/patients/patients/${encodeURIComponent(id)}/profile`); return normalizePatientProfile(raw) } catch (error) { if (!DEMO_MODE) throw error; return getDemoPatientProfile(id) } }
  static async getPrescriptions(patientId: string): Promise<PatientPrescription[]> { try { const result = await this.request<Record<string, unknown>[]>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/prescriptions`); return normalizePatientProfile({ patient: {} as Patient, prescriptions: result }).prescriptions } catch (error) { if (!DEMO_MODE) throw error; return (await this.getPatientProfile(patientId)).prescriptions } }
  static async getTreatmentPlans(patientId: string): Promise<TreatmentPlan[]> { try { return await this.request<TreatmentPlan[]>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/treatment-plans`) } catch (error) { if (!DEMO_MODE) throw error; return [] } }
  static async createTreatmentPlan(patientId: string, input: TreatmentPlanInput): Promise<TreatmentPlan> { try { return await this.request<TreatmentPlan>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/treatment-plans`, { method: 'POST', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; return { id: `demo-treatment-${Date.now()}`, patient_id: patientId, title: input.title, tooth: input.tooth || '', status: input.status || 'recommended', priority: input.priority || 'medium', estimated_cost: input.estimatedCost ?? null, due_date: input.dueDate || null, notes: input.notes || '', created_at: new Date().toISOString(), updated_at: new Date().toISOString() } } }
  static async updateTreatmentPlan(id: string, patientId: string, changes: Partial<TreatmentPlanInput>): Promise<TreatmentPlan> { try { return await this.request<TreatmentPlan>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/treatment-plans/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(changes) }) } catch (error) { if (!DEMO_MODE) throw error; throw error } }
  static async deleteTreatmentPlan(id: string, patientId: string): Promise<void> { try { await this.request<{ id: string }>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/treatment-plans/${encodeURIComponent(id)}`, { method: 'DELETE' }) } catch (error) { if (!DEMO_MODE) throw error } }
  static async getDentalChart(patientId: string): Promise<ToothRecord[]> { try { return await this.request<ToothRecord[]>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/dental-chart`) } catch (error) { if (!DEMO_MODE) throw error; return [] } }
  static async saveToothRecord(patientId: string, toothNumber: number, input: ToothRecordInput): Promise<ToothRecord> { try { return await this.request<ToothRecord>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/dental-chart/teeth/${toothNumber}`, { method: 'PUT', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; return { id: `demo-tooth-${patientId}-${toothNumber}`, patient_id: patientId, tooth_number: toothNumber, conditions: input.conditions || [], pain_level: input.painLevel || 0, status: input.status || 'active', notes: input.notes || '', created_at: new Date().toISOString(), updated_at: new Date().toISOString() } } }
  static async createPrescription(input: PrescriptionInput): Promise<PatientPrescription> { try { const result = await this.request<Record<string, unknown>>(`/api/v1/patients/patients/${encodeURIComponent(input.patientId)}/prescriptions`, { method: 'POST', body: JSON.stringify(input) }); return normalizePatientProfile({ patient: {} as Patient, prescriptions: [result] }).prescriptions[0] } catch (error) { if (!DEMO_MODE) throw error; const profile = await this.getPatientProfile(input.patientId); const prescription: PatientPrescription = { id: `demo-prescription-${Date.now()}`, patient_id: input.patientId, medication: input.medication, dosage: input.dosage, frequency: input.frequency, duration: input.duration, instructions: input.instructions, prescribed_at: new Date().toISOString(), prescribed_by: 'Dr. Rajeev Pratap Singh' }; const profiles = getDemoPatientProfiles().map((item) => item.patient.id === input.patientId ? { ...profile, prescriptions: [prescription, ...profile.prescriptions] } : item); writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles); return prescription } }
  static async updatePrescription(input: PrescriptionUpdateInput): Promise<PatientPrescription> { try { const { patientId, prescriptionId, medication, dosage, frequency, duration, instructions } = input; const result = await this.request<Record<string, unknown>>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/prescriptions/${encodeURIComponent(prescriptionId)}`, { method: 'PATCH', body: JSON.stringify({ medication, dosage, frequency, duration, instructions }) }); return normalizePatientProfile({ patient: {} as Patient, prescriptions: [result] }).prescriptions[0] } catch (error) { if (!DEMO_MODE) throw error; const profiles = getDemoPatientProfiles(); const profile = profiles.find((item) => item.patient.id === input.patientId); if (!profile) throw new Error('Patient profile not found'); const updated = profile.prescriptions.map((item) => item.id === input.prescriptionId ? { ...item, medication: input.medication, dosage: input.dosage, frequency: input.frequency, duration: input.duration, instructions: input.instructions } : item); const nextProfile = { ...profile, prescriptions: updated }; writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles.map((item) => item.patient.id === input.patientId ? nextProfile : item)); const result = updated.find((item) => item.id === input.prescriptionId); if (!result) throw new Error('Prescription not found'); return result } }
  static async deletePrescription(patientId: string, prescriptionId: string): Promise<void> { try { await this.request<{ id: string }>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/prescriptions/${encodeURIComponent(prescriptionId)}`, { method: 'DELETE' }) } catch (error) { if (!DEMO_MODE) throw error; const profiles = getDemoPatientProfiles(); const profile = profiles.find((item) => item.patient.id === patientId); if (!profile) throw new Error('Patient profile not found'); writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles.map((item) => item.patient.id === patientId ? { ...item, prescriptions: item.prescriptions.filter((prescription) => prescription.id !== prescriptionId) } : item)) } }
  static async createMedicalReport(input: MedicalReportInput): Promise<PatientMedicalReport> { try { return await this.request<PatientMedicalReport>(`/api/v1/patients/patients/${encodeURIComponent(input.patientId)}/reports`, { method: 'POST', body: JSON.stringify({ title: input.fileName, attachmentName: input.fileName, attachmentMimeType: input.mimeType, attachmentData: input.dataBase64, description: input.description }) }) } catch (error) { if (!DEMO_MODE) throw error; const profile = await this.getPatientProfile(input.patientId); const report: PatientMedicalReport = { id: `demo-report-${Date.now()}`, patient_id: input.patientId, file_name: input.fileName, mime_type: input.mimeType, file_size: input.fileSize, data_base64: input.dataBase64, description: input.description, uploaded_at: new Date().toISOString(), uploaded_by: 'V.P.S. Dental & Oral Care' }; const profiles = getDemoPatientProfiles().map((item) => item.patient.id === input.patientId ? { ...profile, reports: [report, ...profile.reports] } : item); writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles); return report } }
  static async getMedicalReports(patientId: string): Promise<PatientMedicalReport[]> { try { const reports = await this.request<Array<Record<string, unknown>>>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/reports`); return normalizePatientProfile({ patient: {} as Patient, reports }).reports } catch (error) { if (!DEMO_MODE) throw error; return (await this.getPatientProfile(patientId)).reports } }
  static async deleteMedicalReport(patientId: string, reportId: string): Promise<void> { try { await this.request<{ id: string }>(`/api/v1/patients/patients/${encodeURIComponent(patientId)}/reports/${encodeURIComponent(reportId)}`, { method: 'DELETE' }) } catch (error) { if (!DEMO_MODE) throw error; const profiles = getDemoPatientProfiles(); writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles.map((profile) => profile.patient.id === patientId ? { ...profile, reports: profile.reports.filter((report) => report.id !== reportId) } : profile)) } }
  static async createLabOrder(input: LabOrderInput): Promise<PatientLabOrder> { try { const orderNumber = input.orderNumber || `LAB-${new Date().getFullYear()}-${String(Date.now()).slice(-8)}`; const attachment = makeLabOrderPdf({ orderNumber, patientId: input.patientId, tests: input.tests, teethService: input.teethCreationService, labName: input.labName, instructions: input.instructions }); const result = await this.request<Record<string, unknown>>(`/api/v1/patients/patients/${encodeURIComponent(input.patientId)}/lab-orders`, { method: 'POST', body: JSON.stringify({ ...input, orderNumber, testName: input.tests, copyToPatient: input.copyToPatient, copyToClinic: input.copyToClinic, attachmentName: `${orderNumber}.pdf`, attachmentMimeType: 'application/pdf', attachmentSize: attachment.size, attachmentData: attachment.base64 }) }); return normalizePatientProfile({ patient: {} as Patient, labOrders: [result] }).lab_orders[0] } catch (error) { if (!DEMO_MODE) throw error; const profile = await this.getPatientProfile(input.patientId); const createdAt = new Date().toISOString(); const orderId = `demo-lab-order-${Date.now()}`; const orderNumber = input.orderNumber || `LAB-${new Date().getFullYear()}-${String(Date.now()).slice(-8)}`; const dispatchHistory: PatientDispatchRecord[] = [{ id: `${orderId}-lab`, lab_order_id: orderId, channel: 'lab', recipient: input.labEmail || input.labName, status: 'provider-ready', created_at: createdAt, note: 'Prepared for provider handoff; no external delivery was attempted.' }]; if (input.copyToPatient) dispatchHistory.push({ id: `${orderId}-patient`, lab_order_id: orderId, channel: 'patient', recipient: profile.patient.email || profile.patient.phone || 'Patient contact', status: 'queued', created_at: createdAt, note: 'Patient copy queued for clinic review.' }); if (input.copyToClinic) dispatchHistory.push({ id: `${orderId}-clinic`, lab_order_id: orderId, channel: 'clinic', recipient: 'Clinic records', status: 'queued', created_at: createdAt, note: 'Clinic copy queued for records.' }); const order: PatientLabOrder = { id: orderId, patient_id: input.patientId, order_number: orderNumber, ordered_at: createdAt, tests: input.tests, teeth_creation_service: input.teethCreationService, lab_name: input.labName, lab_email: input.labEmail, lab_phone: input.labPhone, instructions: input.instructions, copy_to_patient: input.copyToPatient, copy_to_clinic: input.copyToClinic, dispatch_status: 'provider-ready', dispatch_note: 'Provider-ready for manual handoff. This demo does not claim real provider delivery.', dispatch_history: dispatchHistory }; const profiles = getDemoPatientProfiles().map((item) => item.patient.id === input.patientId ? { ...profile, lab_orders: [order, ...profile.lab_orders], dispatch_history: [...dispatchHistory, ...profile.dispatch_history] } : item); writeDemoCollection(DEMO_PATIENT_PROFILES_KEY, profiles); return order } }

  static async downloadMedicalReport(patientId: string, reportId: string, fileName: string): Promise<void> {
    try {
      const token = localStorage.getItem('accessToken'); const response = await fetch(`${API_BASE_URL}/api/v1/patients/patients/${encodeURIComponent(patientId)}/reports/${encodeURIComponent(reportId)}/download`, { headers: token ? { Authorization: `Bearer ${token}` } : {} }); if (!response.ok) throw new Error('Unable to download report'); const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = fileName; link.click(); URL.revokeObjectURL(url)
    } catch (error) {
      if (!DEMO_MODE) throw error
      const report = (await this.getPatientProfile(patientId)).reports.find((item) => item.id === reportId); if (!report?.data_base64) throw new Error('Report attachment is not available'); const binary = atob(report.data_base64); const bytes = Uint8Array.from(binary, (character) => character.charCodeAt(0)); const url = URL.createObjectURL(new Blob([bytes], { type: report.mime_type })); const link = document.createElement('a'); link.href = url; link.download = fileName; link.click(); URL.revokeObjectURL(url)
    }
  }

  static async downloadLabOrder(patientId: string, orderId: string, fileName: string): Promise<void> {
    try {
      const token = localStorage.getItem('accessToken'); const response = await fetch(`${API_BASE_URL}/api/v1/patients/patients/${encodeURIComponent(patientId)}/lab-orders/${encodeURIComponent(orderId)}/download`, { headers: token ? { Authorization: `Bearer ${token}` } : {} }); if (!response.ok) throw new Error('Unable to download lab order PDF'); const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = fileName; link.click(); URL.revokeObjectURL(url)
    } catch (error) {
      if (!DEMO_MODE) throw error
      const order = (await this.getPatientProfile(patientId)).lab_orders.find((item) => item.id === orderId); if (!order) throw new Error('Lab order not found'); const pdf = makeLabOrderPdf({ orderNumber: order.order_number, patientId, tests: order.tests, teethService: order.teeth_creation_service, labName: order.lab_name, instructions: order.instructions }); const binary = atob(pdf.base64); const bytes = Uint8Array.from(binary, (character) => character.charCodeAt(0)); const url = URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })); const link = document.createElement('a'); link.href = url; link.download = fileName; link.click(); URL.revokeObjectURL(url)
    }
  }
  static async updateAppointment(id: string, changes: AppointmentUpdateInput): Promise<Appointment> { try { return await this.request<Appointment>(`/api/v1/patients/appointments/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(changes) }) } catch (error) { if (!DEMO_MODE) throw error; const current = getDemoAppointments(); const existing = current.find((item) => item.id === id); if (!existing) throw new Error('Appointment not found'); const updated = { ...existing, appointment_date: changes.appointmentDate ?? existing.appointment_date, appointment_time: changes.appointmentTime ?? existing.appointment_time, duration: changes.duration ?? existing.duration, appointment_type: changes.appointmentType ?? existing.appointment_type, status: changes.status ?? existing.status, reason: changes.reason ?? existing.reason, notes: changes.notes ?? existing.notes }; writeDemoCollection(DEMO_APPOINTMENTS_KEY, current.map((item) => item.id === id ? updated : item)); const patientAppointments = current.map((item) => item.id === id ? updated : item).filter((item) => item.patient_id === updated.patient_id); const nextAppointment = patientAppointments.filter((item) => !['cancelled', 'completed'].includes(item.status) && item.appointment_date >= today).sort((a, b) => `${a.appointment_date} ${a.appointment_time}`.localeCompare(`${b.appointment_date} ${b.appointment_time}`))[0]?.appointment_date || null; const patients = getDemoPatients(); writeDemoCollection(DEMO_PATIENTS_KEY, patients.map((patient) => patient.id === updated.patient_id ? { ...patient, next_appointment: nextAppointment } : patient)); return updated } }
  static async getAppointment(id: string): Promise<Appointment> { try { return await this.request<Appointment>(`/api/v1/patients/appointments/${encodeURIComponent(id)}`) } catch (error) { if (!DEMO_MODE) throw error; const found = getDemoAppointments().find((item) => item.id === id); if (!found) throw new Error('Appointment not found'); return found } }
  static async getAppointments(from: string, to: string): Promise<Appointment[]> { try { return await this.request<Appointment[]>(`/api/v1/patients/appointments?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`) } catch (error) { if (!DEMO_MODE) throw error; return getDemoAppointments().filter((appointment) => appointment.appointment_date >= from && appointment.appointment_date <= to) } }
  static async getTodaysAppointments(): Promise<Appointment[]> { try { return await this.request<Appointment[]>('/api/v1/patients/appointments/today') } catch (error) { if (!DEMO_MODE) throw error; return getDemoAppointments().filter((appointment) => appointment.appointment_date === today) } }
  static async getDentistNotes(): Promise<DentistNote[]> { try { return await this.request<DentistNote[]>('/api/v1/patients/notes') } catch (error) { if (!DEMO_MODE) throw error; return getDemoDentistNotes() } }
  static async createDentistNote(input: DentistNoteInput): Promise<DentistNote> { try { return await this.request<DentistNote>('/api/v1/patients/notes', { method: 'POST', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; const now = new Date().toISOString(); const note: DentistNote = { id: `demo-note-${Date.now()}`, title: input.title, content: input.content, expires_at: input.expiresAt || oneMonthFrom(), is_active: true, created_at: now, updated_at: now }; const next = [note, ...getDemoDentistNotes()]; writeDemoCollection(DEMO_DENTIST_NOTES_KEY, next); return note } }
  static async updateDentistNote(id: string, input: DentistNoteInput): Promise<DentistNote> { try { return await this.request<DentistNote>(`/api/v1/patients/notes/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; const current = getDemoDentistNotes(); const existing = current.find((note) => note.id === id); if (!existing) throw new Error('Dentist note not found'); const note = { ...existing, ...input, expires_at: input.expiresAt || oneMonthFrom(), is_active: true, updated_at: new Date().toISOString() }; writeDemoCollection(DEMO_DENTIST_NOTES_KEY, current.map((item) => item.id === id ? note : item)); return note } }
  static async deleteDentistNote(id: string): Promise<void> { try { await this.request<{ id: string }>(`/api/v1/patients/notes/${encodeURIComponent(id)}`, { method: 'DELETE' }) } catch (error) { if (!DEMO_MODE) throw error; const current = readDemoCollection<DentistNote>(DEMO_DENTIST_NOTES_KEY, []); if (!current.some((note) => note.id === id && note.is_active !== false)) throw new Error('Dentist note not found'); writeDemoCollection(DEMO_DENTIST_NOTES_KEY, current.map((note) => note.id === id ? { ...note, is_active: false, updated_at: new Date().toISOString() } : note)) } }
  static async createBooking(bookingData: unknown) { try { return await this.request<{ patientId: string; appointmentId: string }>('/api/v1/patients/bookings', { method: 'POST', body: JSON.stringify(bookingData) }) } catch (error) { if (!DEMO_MODE) throw error; const input = bookingData as Record<string, unknown>; const timestamp = Date.now(); const patientId = `demo-patient-${timestamp}`; const appointmentId = `demo-appointment-${timestamp}`; const patient: Patient = { id: patientId, first_name: String(input.firstName || ''), last_name: String(input.lastName || ''), phone: String(input.phone || ''), email: String(input.email || ''), gender: String(input.gender || ''), date_of_birth: input.dateOfBirth ? String(input.dateOfBirth) : undefined, last_visit: null, next_appointment: String(input.appointmentDate || '') }; const appointment: Appointment = { id: appointmentId, patient_id: patientId, appointment_date: String(input.appointmentDate || ''), appointment_time: String(input.appointmentTime || ''), duration: Number(input.duration) || 30, appointment_type: String(input.appointmentType || 'Checkup'), status: 'scheduled', reason: String(input.reason || ''), notes: String(input.notes || ''), first_name: patient.first_name, last_name: patient.last_name, phone: patient.phone || '', gender: patient.gender || '' }; writeDemoCollection(DEMO_PATIENTS_KEY, [...getDemoPatients(), patient]); writeDemoCollection(DEMO_APPOINTMENTS_KEY, [...getDemoAppointments(), appointment]); return { patientId, appointmentId } } }
  static async getSettings(): Promise<ClinicSettings | null> { try { return await this.request<ClinicSettings | null>('/api/v1/patients/settings') } catch (error) { if (!DEMO_MODE) throw error; try { return JSON.parse(localStorage.getItem('clinicSettingsDraft') || 'null') as ClinicSettings | null || mockSettings } catch { return mockSettings } } }
  static async updateSettings(settings: Record<string, unknown>): Promise<ClinicSettings> {
    try { return await this.request<ClinicSettings>('/api/v1/patients/settings', { method: 'PUT', body: JSON.stringify(settings) }) }
    catch (error) {
      if (!DEMO_MODE) throw error
      const current = await this.getSettings() || mockSettings
      const next: ClinicSettings = {
        ...current,
        clinic_name: String(settings.clinicName || current.clinic_name),
        clinic_email: settings.clinicEmail === undefined ? current.clinic_email : String(settings.clinicEmail || ''),
        phone: settings.phone === undefined ? current.phone : String(settings.phone || ''),
        address: (settings.address as ClinicSettings['address']) || current.address,
        branding: (settings.branding as ClinicSettings['branding']) || current.branding,
        working_hours: (settings.workingHours as ClinicSettings['working_hours']) || current.working_hours,
        appointment_settings: (settings.appointmentSettings as ClinicSettings['appointment_settings']) || current.appointment_settings,
        notifications: (settings.notifications as ClinicSettings['notifications']) || current.notifications,
        timezone: String(settings.timezone || current.timezone),
      }
      localStorage.setItem('clinicSettingsDraft', JSON.stringify(next))
      return next
    }
  }
  static async getHospitalDirectory(search = ''): Promise<HospitalDirectory[]> { try { return await this.request<HospitalDirectory[]>(`/api/v1/patients/hospitals?search=${encodeURIComponent(search)}`) } catch (error) { if (!DEMO_MODE) throw error; const needle = search.trim().toLowerCase(); return mockHospitals.filter((hospital) => `${hospital.name} ${hospital.relationship} ${hospital.specialty} ${hospital.city}`.toLowerCase().includes(needle)) } }
  static async getClientAlerts(unreadOnly = false): Promise<NotificationAlert[]> { try { return await this.request<NotificationAlert[]>(`/api/v1/patients/notifications?unreadOnly=${unreadOnly}`) } catch (error) { if (!DEMO_MODE) throw error; return getDemoAlerts().filter((alert) => !unreadOnly || !alert.is_read) } }
  static async markClientAlertRead(id: string): Promise<void> { try { await this.request<{ id: string }>(`/api/v1/patients/notifications/${encodeURIComponent(id)}/read`, { method: 'PATCH' }) } catch (error) { if (!DEMO_MODE) throw error; const alerts = getDemoAlerts(); writeDemoCollection(DEMO_CLIENT_ALERTS_KEY, alerts.map((alert) => alert.id === id ? { ...alert, is_read: true } : alert)) } }
  static async getInventory(search = ''): Promise<InventoryItem[]> { try { return await this.request<InventoryItem[]>(`/api/v1/patients/inventory?search=${encodeURIComponent(search)}`) } catch (error) { if (!DEMO_MODE) throw error; return getDemoInventory(search) } }
  static async getInventoryHistory(search = ''): Promise<InventoryItem[]> { try { return await this.request<InventoryItem[]>(`/api/v1/patients/inventory/history?search=${encodeURIComponent(search)}`) } catch (error) { if (!DEMO_MODE) throw error; return getDemoInventory(search, true).filter((item) => item.is_active === false) } }
  static async createInventory(item: InventoryInput): Promise<InventoryItem> { try { return await this.request<InventoryItem>('/api/v1/patients/inventory', { method: 'POST', body: JSON.stringify(item) }) } catch (error) { if (!DEMO_MODE) throw error; const created = inventoryWithStock({ id: `demo-inventory-${Date.now()}`, ...item, reorder_level: item.reorderLevel, is_active: true }); const current = readDemoCollection<InventoryItem>(DEMO_INVENTORY_KEY, mockInventory); writeDemoCollection(DEMO_INVENTORY_KEY, [...current, created]); return created } }
  static async updateInventory(id: string, changes: Partial<InventoryInput>): Promise<InventoryItem> { try { return await this.request<InventoryItem>(`/api/v1/patients/inventory/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(changes) }) } catch (error) { if (!DEMO_MODE) throw error; const current = readDemoCollection<InventoryItem>(DEMO_INVENTORY_KEY, mockInventory); const existing = current.find((item) => item.id === id && item.is_active !== false); if (!existing) throw new Error('Inventory item not found'); const updated = inventoryWithStock({ ...existing, ...changes, reorder_level: changes.reorderLevel ?? existing.reorder_level }); writeDemoCollection(DEMO_INVENTORY_KEY, current.map((item) => item.id === id ? updated : item)); return updated } }
  static async deleteInventory(id: string): Promise<void> { try { await this.request<{ id: string }>(`/api/v1/patients/inventory/${encodeURIComponent(id)}`, { method: 'DELETE' }) } catch (error) { if (!DEMO_MODE) throw error; const current = readDemoCollection<InventoryItem>(DEMO_INVENTORY_KEY, mockInventory); if (!current.some((item) => item.id === id && item.is_active !== false)) throw new Error('Inventory item not found'); writeDemoCollection(DEMO_INVENTORY_KEY, current.map((item) => item.id === id ? { ...item, is_active: false } : item)) } }
  static async getInventoryVendors(search = ''): Promise<InventoryVendor[]> { try { return await this.request<InventoryVendor[]>(`/api/v1/patients/inventory/vendors?search=${encodeURIComponent(search)}`) } catch (error) { if (!DEMO_MODE) throw error; return getDemoInventoryVendors(search) } }
  static async createInventoryVendor(input: InventoryVendorInput): Promise<InventoryVendor> { try { return await this.request<InventoryVendor>('/api/v1/patients/inventory/vendors', { method: 'POST', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; const vendor: InventoryVendor = { id: `demo-vendor-${Date.now()}`, name: input.name, contact_name: input.contactName, email: input.email, phone: input.phone, address: input.address, notes: input.notes, is_active: true, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }; writeDemoCollection(DEMO_INVENTORY_VENDORS_KEY, [...readDemoCollection(DEMO_INVENTORY_VENDORS_KEY, mockInventoryVendors), vendor]); return vendor } }
  static async updateInventoryVendor(id: string, changes: Partial<InventoryVendorInput>): Promise<InventoryVendor> { try { return await this.request<InventoryVendor>(`/api/v1/patients/inventory/vendors/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(changes) }) } catch (error) { if (!DEMO_MODE) throw error; const vendors = readDemoCollection<InventoryVendor>(DEMO_INVENTORY_VENDORS_KEY, mockInventoryVendors); const existing = vendors.find((vendor) => vendor.id === id && vendor.is_active !== false); if (!existing) throw new Error('Inventory vendor not found'); const vendor = { ...existing, name: changes.name ?? existing.name, contact_name: changes.contactName === undefined ? existing.contact_name : changes.contactName, email: changes.email === undefined ? existing.email : changes.email, phone: changes.phone === undefined ? existing.phone : changes.phone, address: changes.address === undefined ? existing.address : changes.address, notes: changes.notes === undefined ? existing.notes : changes.notes, updated_at: new Date().toISOString() }; writeDemoCollection(DEMO_INVENTORY_VENDORS_KEY, vendors.map((item) => item.id === id ? vendor : item)); return vendor } }
  static async getInventoryOrders(options: { search?: string; status?: InventoryOrderStatus; history?: boolean } | string = {}): Promise<InventoryOrder[]> { const normalized: { search?: string; status?: InventoryOrderStatus; history?: boolean } = typeof options === 'string' ? { search: options } : options; try { const query = new URLSearchParams(); if (normalized.search) query.set('search', normalized.search); if (normalized.status) query.set('status', normalized.status); return await this.request<InventoryOrder[]>(`/api/v1/patients/inventory/orders${normalized.history ? '/history' : ''}${query.toString() ? `?${query}` : ''}`) } catch (error) { if (!DEMO_MODE) throw error; return getDemoInventoryOrders(normalized) } }
  static async getInventoryOrder(id: string): Promise<InventoryOrder> { try { return await this.request<InventoryOrder>(`/api/v1/patients/inventory/orders/${encodeURIComponent(id)}`) } catch (error) { if (!DEMO_MODE) throw error; const order = getDemoInventoryOrders().find((item) => item.id === id); if (!order) throw new Error('Inventory order not found'); return order } }
  static async createInventoryOrder(input: InventoryOrderInput): Promise<InventoryOrder> { try { return await this.request<InventoryOrder>('/api/v1/patients/inventory/orders', { method: 'POST', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; const now = new Date().toISOString(); const vendor = input.vendorId ? getDemoInventoryVendors().find((item) => item.id === input.vendorId) : undefined; const items = input.items.map(demoOrderItem); const subtotal = items.reduce((sum, item) => sum + item.total_price, 0); const order: InventoryOrder = { id: `demo-order-${Date.now()}`, vendor_id: input.vendorId || null, vendor_name: vendor?.name || null, vendor_email: vendor?.email || null, vendor_phone: vendor?.phone || null, order_number: input.orderNumber || `PO-DEMO-${String(Date.now()).slice(-8)}`, patient_id: input.patientId || null, status: input.status || 'draft', payment_status: input.paymentStatus || 'pending', payment_method: input.paymentMethod || null, subtotal, tax_amount: input.taxAmount || 0, total_amount: subtotal + (input.taxAmount || 0), notes: input.notes || '', expected_delivery_date: input.expectedDeliveryDate || null, placed_at: input.status && input.status !== 'draft' ? now : null, created_at: now, updated_at: now, items, events: [], communications: [] }; if (order.status !== 'draft') order.events.push({ id: `${order.id}-placed`, order_id: order.id, event_type: 'order_placed', status: order.status, description: 'Order placed for vendor processing.', location: '', occurred_at: now }); writeDemoCollection(DEMO_INVENTORY_ORDERS_KEY, [order, ...getDemoInventoryOrders()]); return order } }
  static async updateInventoryOrder(id: string, changes: InventoryOrderUpdateInput): Promise<InventoryOrder> { try { return await this.request<InventoryOrder>(`/api/v1/patients/inventory/orders/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(changes) }) } catch (error) { if (!DEMO_MODE) throw error; const orders = getDemoInventoryOrders(); const existing = orders.find((order) => order.id === id); if (!existing) throw new Error('Inventory order not found'); const items = changes.items ? changes.items.map(demoOrderItem) : existing.items; const subtotal = changes.items ? items.reduce((sum, item) => sum + item.total_price, 0) : existing.subtotal; const status = changes.status || existing.status; const now = new Date().toISOString(); const updated: InventoryOrder = { ...existing, ...changes, vendor_id: changes.vendorId === undefined ? existing.vendor_id : changes.vendorId, patient_id: changes.patientId === undefined ? existing.patient_id : changes.patientId, order_number: changes.orderNumber || existing.order_number, status, payment_status: changes.paymentStatus || existing.payment_status, payment_method: changes.paymentMethod === undefined ? existing.payment_method : changes.paymentMethod, tax_amount: changes.taxAmount ?? existing.tax_amount, subtotal, total_amount: subtotal + (changes.taxAmount ?? existing.tax_amount), expected_delivery_date: changes.expectedDeliveryDate === undefined ? existing.expected_delivery_date : changes.expectedDeliveryDate, items, updated_at: now, placed_at: status !== 'draft' ? existing.placed_at || now : existing.placed_at }; if (changes.status && changes.status !== existing.status) updated.events = [{ id: `${id}-event-${Date.now()}`, order_id: id, event_type: 'status_updated', status: changes.status, description: `Order status changed to ${changes.status}.`, location: '', occurred_at: now }, ...existing.events]; writeDemoCollection(DEMO_INVENTORY_ORDERS_KEY, orders.map((order) => order.id === id ? updated : order)); return updated } }
  static async addInventoryOrderEvent(orderId: string, input: { eventType: string; status?: InventoryOrderStatus; description?: string; location?: string; occurredAt?: string }): Promise<InventoryOrderEvent> { try { return await this.request<InventoryOrderEvent>(`/api/v1/patients/inventory/orders/${encodeURIComponent(orderId)}/events`, { method: 'POST', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; const orders = getDemoInventoryOrders(); const order = orders.find((item) => item.id === orderId); if (!order) throw new Error('Inventory order not found'); const event: InventoryOrderEvent = { id: `demo-order-event-${Date.now()}`, order_id: orderId, event_type: input.eventType, status: input.status || null, description: input.description || '', location: input.location || '', occurred_at: input.occurredAt || new Date().toISOString() }; order.events = [event, ...order.events]; if (input.status) order.status = input.status; order.updated_at = new Date().toISOString(); writeDemoCollection(DEMO_INVENTORY_ORDERS_KEY, orders); return event } }
  static async sendInventoryOrderCommunication(orderId: string, input: InventoryCommunicationInput): Promise<InventoryCommunication[]> { try { return await this.request<InventoryCommunication[]>(`/api/v1/patients/inventory/orders/${encodeURIComponent(orderId)}/communications`, { method: 'POST', body: JSON.stringify(input) }) } catch (error) { if (!DEMO_MODE) throw error; const orders = getDemoInventoryOrders(); const order = orders.find((item) => item.id === orderId); if (!order) throw new Error('Inventory order not found'); const vendor = order.vendor_id ? getDemoInventoryVendors().find((item) => item.id === order.vendor_id) : undefined; const patient = getDemoPatients().find((item) => item.id === (input.patientId || order.patient_id)); const patientRecipient = input.channel === 'email' ? patient?.email : patient?.phone; const now = new Date().toISOString(); const recipient = input.recipient || (input.recipientType === 'patient' || input.recipientType === 'client' ? patientRecipient : input.channel === 'email' ? vendor?.email : vendor?.phone); if (!recipient) throw new Error(input.recipientType === 'patient' || input.recipientType === 'client' ? 'Patient contact is required' : 'Vendor contact is required'); const records: InventoryCommunication[] = [{ id: `demo-communication-${Date.now()}`, order_id: orderId, channel: input.channel, recipient_type: input.recipientType || 'vendor', recipient, subject: input.subject || 'Inventory purchase order', body: input.body, status: 'queued', sent_at: null, created_at: now }]; if (input.copyToPatient) { if (!patientRecipient) throw new Error('Patient contact is required for a copy'); records.push({ ...records[0], id: `demo-communication-${Date.now()}-patient`, recipient_type: 'patient', recipient: patientRecipient, created_at: now }) } order.communications = [...records, ...order.communications]; order.updated_at = now; writeDemoCollection(DEMO_INVENTORY_ORDERS_KEY, orders); return records } }
  static async reorderInventoryOrder(orderId: string): Promise<InventoryOrder> { try { return await this.request<InventoryOrder>(`/api/v1/patients/inventory/orders/${encodeURIComponent(orderId)}/reorder`, { method: 'POST' }) } catch (error) { if (!DEMO_MODE) throw error; const source = getDemoInventoryOrders().find((order) => order.id === orderId && order.status !== 'draft'); if (!source) throw new Error('Historical inventory order not found'); const now = new Date().toISOString(); const order: InventoryOrder = { ...source, id: `demo-order-${Date.now()}`, order_number: `PO-REORDER-${String(Date.now()).slice(-10)}`, status: 'draft', payment_status: 'pending', payment_method: null, notes: `Reordered from ${source.order_number}. ${source.notes}`.trim(), placed_at: null, created_at: now, updated_at: now, items: source.items.map((item, index) => ({ ...item, id: `demo-order-item-${Date.now()}-${index}` })), events: [], communications: [] }; writeDemoCollection(DEMO_INVENTORY_ORDERS_KEY, [order, ...getDemoInventoryOrders()]); return order } }
  static async getSupportChat(): Promise<SupportChat | null> { try { return await this.request<SupportChat | null>('/api/v1/patients/support/chat') } catch (error) { if (!DEMO_MODE) throw error; return readDemoCollection<SupportChat>(DEMO_SUPPORT_CHAT_KEY, [])[0] || null } }
  static async getSupportConversations(options: { date?: string; status?: 'open' | 'closed' } = {}): Promise<SupportConversationSummary[]> {
    try {
      const query = new URLSearchParams()
      if (options.date) query.set('date', options.date)
      if (options.status) query.set('status', options.status)
      return await this.request<SupportConversationSummary[]>(`/api/v1/patients/support/conversations${query.size ? `?${query}` : ''}`)
    } catch (error) {
      if (!DEMO_MODE) throw error
      return readDemoCollection<SupportChat>(DEMO_SUPPORT_CHAT_KEY, [])
        .filter((chat) => {
          const conversation = chat.conversation
          if (!conversation) return false
          if (options.status && conversation.status !== options.status) return false
          if (options.date && !conversation.updated_at.startsWith(options.date)) return false
          return true
        })
        .sort((a, b) => {
          const openDelta = Number(b.conversation?.status === 'open') - Number(a.conversation?.status === 'open')
          if (openDelta) return openDelta
          return (b.conversation?.updated_at || '').localeCompare(a.conversation?.updated_at || '')
        })
        .map((chat) => {
          const last = chat.messages[chat.messages.length - 1]
          const subject = chat.conversation!.subject
          const named = subject.includes(' · ') ? subject.split(' · ')[0] : subject
          return { ...chat.conversation!, requester_name: named || 'Clinic thread', last_message: last?.body || null }
        })
    }
  }
  static async updateSupportConversationStatus(id: string, status: 'open' | 'closed'): Promise<SupportConversation> {
    try { return await this.request<SupportConversation>(`/api/v1/patients/support/conversations/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify({ status }) }) }
    catch (error) {
      if (!DEMO_MODE) throw error
      const chats = readDemoCollection<SupportChat>(DEMO_SUPPORT_CHAT_KEY, [])
      const chat = chats.find((item) => item.conversation?.id === id)
      if (!chat?.conversation) throw new Error('Support conversation not found')
      chat.conversation = { ...chat.conversation, status, updated_at: new Date().toISOString() }
      writeDemoCollection(DEMO_SUPPORT_CHAT_KEY, chats)
      return chat.conversation
    }
  }
  static async getSupportConversation(id: string): Promise<SupportChat> { try { return await this.request<SupportChat>(`/api/v1/patients/support/conversations/${encodeURIComponent(id)}`) } catch (error) { if (!DEMO_MODE) throw error; const chat = readDemoCollection<SupportChat>(DEMO_SUPPORT_CHAT_KEY, []).find((item) => item.conversation?.id === id); if (!chat) throw new Error('Support conversation not found'); return chat } }
  static async createSupportConversation(subject = 'Hospital support'): Promise<SupportChat> {
    try { return await this.request<SupportChat>('/api/v1/patients/support/conversations', { method: 'POST', body: JSON.stringify({ subject }) }) }
    catch (error) {
      if (!DEMO_MODE) throw error
      const now = new Date().toISOString()
      const chat: SupportChat = { conversation: { id: `demo-support-${Date.now()}`, subject, status: 'open', created_by: 'demo-user', created_at: now, updated_at: now }, messages: [] }
      const chats = readDemoCollection<SupportChat>(DEMO_SUPPORT_CHAT_KEY, []).filter((item) => item.conversation?.id !== chat.conversation?.id)
      writeDemoCollection(DEMO_SUPPORT_CHAT_KEY, [chat, ...chats])
      return chat
    }
  }
  static async sendSupportMessage(conversationId: string, body: string): Promise<SupportMessage> { try { return await this.request<SupportMessage>(`/api/v1/patients/support/conversations/${encodeURIComponent(conversationId)}/messages`, { method: 'POST', body: JSON.stringify({ body }) }) } catch (error) { if (!DEMO_MODE) throw error; const chats = readDemoCollection<SupportChat>(DEMO_SUPPORT_CHAT_KEY, []); const chat = chats.find((item) => item.conversation?.id === conversationId); if (!chat || !chat.conversation) throw new Error('Support conversation not found'); const message: SupportMessage = { id: `demo-support-message-${Date.now()}`, conversation_id: conversationId, sender_id: 'demo-user', sender_name: 'You', sender_role: 'client', body, created_at: new Date().toISOString() }; chat.messages = [...chat.messages, message]; chat.conversation.updated_at = message.created_at; writeDemoCollection(DEMO_SUPPORT_CHAT_KEY, chats); return message } }

  static async getPayments(options: { search?: string; status?: PaymentStatus; customerId?: string } = {}): Promise<Payment[]> {
    try { const query = new URLSearchParams(); if (options.search) query.set('search', options.search); if (options.status) query.set('status', options.status); if (options.customerId) query.set('customerId', options.customerId); return await this.request<Payment[]>(`/api/v1/patients/payments${query.toString() ? `?${query}` : ''}`) }
    catch (error) { if (!DEMO_MODE) throw error; const needle = (options.search || '').trim().toLowerCase(); return getDemoPayments().map(demoPaymentWithCustomer).filter((payment) => (!options.status || payment.status === options.status) && (!options.customerId || payment.customer_id === options.customerId) && (!needle || `${payment.payment_number} ${payment.first_name} ${payment.last_name}`.toLowerCase().includes(needle))).sort((a, b) => b.created_at.localeCompare(a.created_at)) }
  }
  static async getInvoices(options: { customerId?: string; status?: InvoiceStatus } = {}): Promise<Invoice[]> { const query = new URLSearchParams(); if (options.customerId) query.set('customerId', options.customerId); if (options.status) query.set('status', options.status); return this.request<Invoice[]>(`/api/v1/patients/invoices${query.size ? `?${query}` : ''}`) }
  static async createInvoice(input: InvoiceInput): Promise<Invoice> { return this.request<Invoice>('/api/v1/patients/invoices', { method: 'POST', body: JSON.stringify(input) }) }
  static async updateInvoice(id: string, input: InvoiceUpdateInput): Promise<Invoice> { return this.request<Invoice>(`/api/v1/patients/invoices/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }) }
  static async deleteInvoice(id: string): Promise<void> { await this.request<{ id: string }>(`/api/v1/patients/invoices/${encodeURIComponent(id)}`, { method: 'DELETE' }) }
  static async getMessageCampaigns(status?: MessageCampaignStatus): Promise<MessageCampaign[]> { return this.request<MessageCampaign[]>(`/api/v1/patients/message-campaigns${status ? `?status=${encodeURIComponent(status)}` : ''}`) }
  static async createMessageCampaign(input: MessageCampaignInput): Promise<MessageCampaign> { return this.request<MessageCampaign>('/api/v1/patients/message-campaigns', { method: 'POST', body: JSON.stringify(input) }) }
  static async updateMessageCampaign(id: string, input: Partial<Omit<MessageCampaignInput, 'recipients'>> & { status?: MessageCampaignStatus }): Promise<MessageCampaign> { return this.request<MessageCampaign>(`/api/v1/patients/message-campaigns/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }) }
  static async queueMessageCampaign(id: string): Promise<{ campaign: MessageCampaign; delivery: { provider: 'twilio' | 'vonage' | null; configured: boolean } }> { return this.request<{ campaign: MessageCampaign; delivery: { provider: 'twilio' | 'vonage' | null; configured: boolean } }>(`/api/v1/patients/message-campaigns/${encodeURIComponent(id)}/queue`, { method: 'POST' }) }
  static async deleteMessageCampaign(id: string): Promise<void> { await this.request<{ id: string }>(`/api/v1/patients/message-campaigns/${encodeURIComponent(id)}`, { method: 'DELETE' }) }
  static async createPayment(input: PaymentInput): Promise<Payment> {
    try { return await this.request<Payment>('/api/v1/patients/payments', { method: 'POST', body: JSON.stringify(input) }) }
    catch (error) { if (!DEMO_MODE) throw error; const customer = getDemoPatients().find((patient) => patient.id === input.customerId); if (!customer) throw new Error('Customer not found'); const now = new Date().toISOString(); const status = input.status || 'pending'; const payment: Payment = { id: `demo-payment-${Date.now()}`, customer_id: input.customerId, payment_number: input.paymentNumber || `PAY-${new Date().getFullYear()}-${String(Date.now()).slice(-8)}`, amount: input.amount, currency: input.currency || 'INR', status, method: input.method || null, description: input.description || '', paid_at: status === 'paid' ? now : null, created_at: now, updated_at: now, first_name: customer.first_name, last_name: customer.last_name, email: customer.email ?? null, phone: customer.phone ?? null }; writeDemoCollection(DEMO_PAYMENTS_KEY, [payment, ...getDemoPayments()]); return payment }
  }
  static async updatePayment(id: string, changes: PaymentUpdateInput): Promise<Payment> {
    try { return await this.request<Payment>(`/api/v1/patients/payments/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(changes) }) }
    catch (error) { if (!DEMO_MODE) throw error; const payments = getDemoPayments(); const existing = payments.find((payment) => payment.id === id); if (!existing) throw new Error('Payment not found'); const now = new Date().toISOString(); const status = changes.status ?? existing.status; const updated: Payment = { ...existing, amount: changes.amount ?? existing.amount, currency: changes.currency ?? existing.currency, status, method: changes.method === undefined ? existing.method : changes.method, description: changes.description ?? existing.description, paid_at: status === 'paid' ? existing.paid_at || now : existing.paid_at, updated_at: now }; writeDemoCollection(DEMO_PAYMENTS_KEY, payments.map((payment) => payment.id === id ? updated : payment)); return demoPaymentWithCustomer(updated) }
  }
  static async getPaymentSummary(): Promise<PaymentSummary> {
    try { return await this.request<PaymentSummary>('/api/v1/patients/payments/summary') }
    catch (error) { if (!DEMO_MODE) throw error; const payments = getDemoPayments(); const sum = (predicate: (payment: Payment) => boolean) => payments.filter(predicate).reduce((total, payment) => total + Number(payment.amount || 0), 0); return { total_payments: payments.length, total_amount: sum(() => true), collected_amount: sum((payment) => payment.status === 'paid'), pending_amount: sum((payment) => payment.status === 'pending'), refunded_amount: sum((payment) => payment.status === 'refunded'), paid_count: payments.filter((payment) => payment.status === 'paid').length, pending_count: payments.filter((payment) => payment.status === 'pending').length, customers_count: new Set(payments.map((payment) => payment.customer_id)).size } }
  }
  static async downloadPaymentReport(clinicName: string): Promise<void> {
    const [summary, payments] = await Promise.all([this.getPaymentSummary(), this.getPayments()])
    const pdf = makePaymentReportPdf({ clinicName, generatedAt: new Date().toLocaleString('en-IN'), summary, payments })
    const binary = atob(pdf.base64); const bytes = Uint8Array.from(binary, (character) => character.charCodeAt(0)); const url = URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })); const link = document.createElement('a'); link.href = url; link.download = `payment-report-${localDateKey(new Date())}.pdf`; link.click(); URL.revokeObjectURL(url)
  }

  private static async requestBody<T>(path: string, options: RequestInit = {}): Promise<T> {
    const { ok, status, body } = await this.fetchJson(path, options)
    if (!ok) throw new ApiError(typeof body.message === 'string' ? body.message : `Request failed (${status || 'network'})`, status)
    return body as T
  }
}