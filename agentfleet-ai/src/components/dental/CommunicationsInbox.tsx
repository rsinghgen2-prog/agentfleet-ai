...
export default function CommunicationsInbox() {
  ...
  const loadConversations = async () => {
    ...
    catch (reason) { setError(describeApiError(reason, "Unable to load today's communications from the backend.")) }
  ...
  useEffect(() => { if (selectedId) void DashboardService.getSupportConversation(selectedId).then(setChat).catch((reason) => setError(describeApiError(reason, 'Unable to load conversation.'))) }, [selectedId])
  ...
  const send = async (event: FormEvent) => {
    ...
    catch (reason) { setError(describeApiError(reason, 'Unable to send message')) }
    finally { setSending(false) }
  }
  const newConversation = async () => { try { const created = await DashboardService.createSupportConversation('Patient communication'); setConversations((current) => [created.conversation!, ...current]); setSelectedId(created.conversation!.id); setChat(created) } catch (reason) { setError(describeApiError(reason, 'Unable to create conversation')) } }
