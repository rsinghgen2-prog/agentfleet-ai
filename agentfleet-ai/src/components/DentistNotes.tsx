...
import { DashboardService, type DentistNote } from '../services/dashboardService'

type NoteDraft = Pick<DentistNote, 'title' | 'content'>

const emptyDraft: NoteDraft = { title: '', content: '' }
const formatNoteDate = (value: string) => new Date(value).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
  ...
  const saveNote = async (event: FormEvent) => {
    ...
    try {
      const saved = editingId
        ? await DashboardService.updateDentistNote(editingId, { title, content })
        : await DashboardService.createDentistNote({ title, content })
